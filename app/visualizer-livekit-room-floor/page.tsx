...
export default function LiveKitRoomFloorPreviewPage() {
    ...
    floorOwner === "candidate"
      ...
      : phase === "reviewing"
        ...
        : phase === "closing"
          ? "#3D9B8B"
          : "#E6B65C";
  const turnSideX = floorOwner === "candidate" ? "98%" : floorOwner === "ai" ? "2%" : "50%";
  const turnSideY = floorOwner === "candidate" ? "55%" : floorOwner === "ai" ? "48%" : "52%";
  const turnSideXb = floorOwner === "candidate" ? "78%" : floorOwner === "ai" ? "22%" : "50%";
  const turnSideYb = floorOwner === "candidate" ? "43%" : floorOwner === "ai" ? "60%" : "52%";
  const turnSideStrength = floorOwner === "ready" ? 0 : clamp(0.2 + energy * 0.28, 0.16, 0.44);
  const floorLabel =
    floorOwner === "candidate"
      ? "Candidate turn"
      : floorOwner === "ai"
  ...
  function startEnergy(kind: "ai" | "candidate" = "candidate") {
    if (energyRef.current) window.clearInterval(energyRef.current);
    if (kind === "candidate" && micStreamRef.current) return;
    let tick = 0;
  ...
  async function startMicrophone() {
    ...
    try {
      ...
      setLiveAnswer("Mic is live. Speak to test candidate-side energy.");
      ...
      const readMic = () => {
        ...
        const next = clamp(0.12 + Math.pow(rms, 0.58) * 2.8, 0.1, 0.92);

        if (phaseRef.current === "listening" || phaseRef.current === "ready") {
          setEnergy((current) => current * 0.68 + next * 0.32);
        }
        micFrameRef.current = window.requestAnimationFrame(readMic);
      };
      readMic();
    } catch {
      setMicOn(false);
      setMicError("Microphone blocked. The room will keep using simulated energy.");
    }
  }

  function stopMicrophone(updateState = true) {
      ...
      style={
        {
          ...
          "--mural-b": mural.b,
          "--mural-c": mural.c,
          "--turn-side-glow": turnSideGlow,
          "--turn-side-glow-b": turnSideGlowB,
          "--turn-side-x": turnSideX,
          "--turn-side-y": turnSideY,
          "--turn-side-x-b": turnSideXb,
          "--turn-side-y-b": turnSideYb,
          "--turn-side-strength": turnSideStrength.toFixed(3),
        } as CSSProperties
      }
    >
      <style>{`
        ...
        .lk-room[data-floor="ai"] .lk-presence {
          ...
          box-shadow:
            -4px 0 0 color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 32%), transparent),
            -22px 0 46px -18px color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 62%), transparent),
            0 -16px 34px -24px color-mix(in oklch, var(--turn-side-glow-b) calc(var(--turn-side-strength) * 46%), transparent),
            0 16px 34px -24px color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 40%), transparent),
            0 0 28px color-mix(in oklch, var(--mural-a) 7%, transparent),
            inset 0 0 0 1px color-mix(in oklch, var(--mural-a) 8%, transparent);
        }
        .lk-label {
          margin: 0;
        ...
        .lk-mini-btn.active {
          border-color: color-mix(in oklch, var(--mural-a) 42%, rgba(255,255,255,0.16));
          background: color-mix(in oklch, var(--mural-a) 12%, rgba(255,255,255,0.055));
          box-shadow: 0 0 18px color-mix(in oklch, var(--mural-a) 18%, transparent);
        }
        .lk-meter-note {
          margin: 0;
          color: rgba(255,255,255,0.42);
        ...
        .lk-floor-endpoint.ai::before {
          background: rgba(230,182,92,0.38);
        }
        .lk-floor-endpoint.candidate::before {
          background: rgba(74,191,224,0.38);
        }
        .lk-room[data-floor="ai"] .lk-floor-endpoint.ai {
          color: rgba(255,255,255,0.82);
          border-color: rgba(230,182,92,0.28);
          background: rgba(230,182,92,0.065);
          box-shadow: 0 0 22px rgba(217,130,59,0.13);
        }
        .lk-room[data-floor="candidate"] .lk-floor-endpoint.candidate {
          color: rgba(255,255,255,0.82);
          border-color: rgba(74,191,224,0.28);
          background: rgba(74,191,224,0.065);
          box-shadow: 0 0 22px rgba(47,111,173,0.14);
        }
        ...
        .lk-room[data-floor="candidate"] .lk-floor-endpoint.candidate::before {
          background: #4abfe0;
          box-shadow: 0 0 18px rgba(47,111,173,0.58);
        }
        .lk-floor-track {
          height: 3px;
          border-radius: 999px;
        ...
        .lk-floor-track.ai::before {
          background: linear-gradient(90deg, rgba(217,130,59,0.1), rgba(230,182,92,0.9));
          box-shadow: 0 0 16px rgba(217,130,59,0.34);
        }
        .lk-floor-track.candidate::before {
          background: linear-gradient(90deg, rgba(74,191,224,0.9), rgba(47,111,173,0.12));
          box-shadow: 0 0 16px rgba(47,111,173,0.34);
        }
        .lk-floor-track.ai::before {
        ...
        .lk-memory {
          ...
          overflow: hidden;
          padding: 18px;
          transition: padding 220ms ease, border-color 180ms ease, box-shadow 180ms ease;
        }
        .lk-room[data-floor="candidate"] .lk-memory {
          border-color: color-mix(in oklch, var(--mural-a) 18%, rgba(255,255,255,0.08));
          box-shadow:
            4px 0 0 color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 28%), transparent),
            22px 0 46px -18px color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 46%), transparent),
            0 -16px 34px -24px color-mix(in oklch, var(--turn-side-glow-b) calc(var(--turn-side-strength) * 30%), transparent),
            0 16px 34px -24px color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 28%), transparent),
            inset 0 0 0 1px color-mix(in oklch, var(--mural-a) 6%, transparent);
        }
        .lk-memory.collapsed {
          display: flex;
          align-items: center;
        ...
        .lk-room[data-floor="candidate"] .lk-candidate-camera {
          ...
          box-shadow:
            3px 0 0 color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 32%), transparent),
            18px 0 42px -18px color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 58%), transparent),
            0 -14px 30px -22px color-mix(in oklch, var(--turn-side-glow-b) calc(var(--turn-side-strength) * 42%), transparent),
            0 14px 30px -22px color-mix(in oklch, var(--turn-side-glow) calc(var(--turn-side-strength) * 36%), transparent),
            0 0 24px color-mix(in oklch, var(--mural-a) 8%, transparent),
          ...
          <aside className="lk-presence lk-glass">
            ...
            <div className="lk-meter">
              <div className="lk-meter-row">
                ...
                <span>{micOn ? "real mic" : phase === "listening" ? "sim live" : mural.label}</span>
              </div>
              <div className="lk-meter-bar">
                <div className="lk-meter-fill" />
              </div>
              <div className="lk-meter-actions">
                <button
                  className={["lk-mini-btn", micOn ? "active" : ""].join(" ")}
                  type="button"
                  onClick={toggleMicrophone}
                >
                  {micOn ? "Stop mic" : "Test mic"}
                </button>
                <p className="lk-meter-note">{micOn ? "Speak to drive the candidate glow." : "Real mic test."}</p>
              </div>
              {micError && <p className="lk-meter-error">{micError}</p>}
            </div>
          </aside>
